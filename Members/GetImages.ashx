...
public class GetImages : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        if (context.Request.QueryString[0] != null)
        {
            ...
            {
                string sProfileId = context.Request.QueryString[0].ToString();
                if (!string.IsNullOrEmpty(sProfileId))
                {
                    picid = Convert.ToInt32(sProfileId);
                }
            }
            ...
            if (picid > 0)
            {
                try
                {
                    string strConn = GetConnectionString("MatchElfConnectionString");
                    string sql = "SELECT TOP 1 ImageData, ContentType FROM [MatchElf].[dbo].[Images] WHERE (LEN(ContentType) > 1) " +
                            " AND (ImageId = " + picid + ") ORDER BY PrimaryImage DESC";
    ...
    public Stream ShowInboxImage(int picid)
    {
        ...
        string sql = "SELECT ImageData FROM Images WHERE ImageId = " + picid.ToString();
